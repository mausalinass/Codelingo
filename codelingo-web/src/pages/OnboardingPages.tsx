...
import { CountryFlag, LanguageTrackIcon } from "../lib/icons";
...
const isFrench = (language: UiLanguage | null) => language === "fr";

export function LanguageStep() {
  const { state, update } = useOnboarding();
  const navigate = useNavigate();
  const options = UI_LOCALES.map(locale => ({ ...locale, detail: locale.id === "fr" ? "Explications et aide en français" : locale.id === "es" ? "Explicaciones y ayuda en español" : locale.id === "en" ? "Lessons and help in English" : `Coding explanations in ${locale.label}` }));
  return <OnboardingShell step={1} backTo="/">
    ...
    <div className="grid max-h-[30rem] gap-4 overflow-y-auto pr-2 sm:grid-cols-2 custom-scrollbar">{options.map(o => <button key={o.id} onClick={() => update({ uiLanguage: o.id })} className={`${optionClass(state.uiLanguage === o.id)} flex items-center gap-4`}><CountryFlag code={o.flag} className="h-8 w-12 shrink-0"/><span><span className="text-xl">{o.label}</span><span className="mt-1 block text-sm text-slate-500">{o.detail}</span></span>{state.uiLanguage === o.id && <Check className="absolute right-4 top-4 text-red-600" />}</button>)}</div>
    <button disabled={!state.uiLanguage} onClick={() => navigate("/onboarding/track")} className={`${primaryButton} mt-8`}>{state.uiLanguage === "fr" ? "CONTINUER" : state.uiLanguage === "es" ? "CONTINUAR" : "CONTINUE"}</button>
  </OnboardingShell>;
}
...
export function TrackStep() {
  ...
  const fr = isFrench(state.uiLanguage);
  return <OnboardingShell step={2} backTo="/onboarding/language">
    <LouisPrompt>{fr ? "Que veux-tu programmer en premier ?" : es ? "¿Qué quieres programar primero?" : "What do you want to program first?"}</LouisPrompt>
    ...
    <h1 className="mb-8 text-center text-3xl font-black sm:text-4xl">{fr ? "Que veux-tu programmer ?" : es ? "¿Qué quieres programar?" : "Choose your programming track"}</h1>
    ...
    <p className="mt-5 text-center text-sm text-slate-500">{fr ? "Tu pourras changer de parcours plus tard. Go, Rust, Java et C++ sont disponibles dans le tableau de bord." : es ? "Podrás cambiar de ruta después. Go, Rust, Java y C++ están disponibles en el dashboard." : "You can change tracks later. Go, Rust, Java and C++ remain available from the dashboard."}</p>
    <button disabled={!state.programmingLanguage} onClick={() => navigate("/onboarding/experience")} className={`${primaryButton} mt-6`}>{fr ? "CONTINUER" : es ? "CONTINUAR" : "CONTINUE"}</button>
  </OnboardingShell>;
}
...
export function ExperienceStep() {
  ...
  const es = isSpanish(state.uiLanguage);
  const fr = isFrench(state.uiLanguage);
  const levels: { id: ExperienceLevel; label: string; detail: string; icon: typeof Sprout }[] = fr ? [
    { id: "beginner", label: "Je commence de zéro", detail: "Je n'ai jamais programmé.", icon: Sprout },
    { id: "basic", label: "Je connais les variables et les conditions", detail: "J'ai déjà vu les bases.", icon: BarChart3 },
    { id: "intermediate", label: "J'écris des fonctions et des boucles", detail: "J'ai un peu de pratique.", icon: Braces },
    { id: "project_experience", label: "J'ai créé mes propres projets", detail: "Je suis à l'aise pour programmer.", icon: Trophy },
  ] : es ? [
  ...
  return <OnboardingShell step={3} backTo="/onboarding/track">
    <LouisPrompt>{fr ? "Quel est ton niveau en programmation ?" : es ? "¿Cuánto sabes de programación?" : "How much programming do you already know?"}</LouisPrompt>
    ...
    <p className="mt-5 text-sm font-semibold text-slate-500">ⓘ {fr ? "Ensuite, Louis te proposera un test rapide." : es ? "Después haremos una prueba breve." : "Next, Louis will give you a short test."}</p>
    <button disabled={!state.experienceLevel} onClick={() => navigate("/onboarding/placement")} className={`${primaryButton} mt-6`}>{fr ? "COMMENCER LE TEST DE 5 QUESTIONS" : es ? "COMENZAR PRUEBA DE 5 PREGUNTAS" : "START THE 5-QUESTION TEST"}</button>
  </OnboardingShell>;
}
...
export function PlacementStep() {
  ...
  return <OnboardingShell step={4} backTo="/onboarding/experience">
    <div className="mb-4 flex items-center justify-between font-bold text-slate-500"><span>{fr ? `Question ${index + 1} sur 5` : es ? `Pregunta ${index + 1} de 5` : `Question ${index + 1} of 5`}</span><span className="flex items-center gap-2"><LanguageTrackIcon languageId={state.programmingLanguage} className="h-6 w-6" />{SUPPORTED_LANGUAGES[state.programmingLanguage].label}</span></div>
    <LouisPrompt>{fr ? "Tu peux le faire ! Réfléchis bien." : es ? "¡Tú puedes! Piénsalo bien." : q.prompt}</LouisPrompt><h1 className="mb-4 text-2xl font-black">{fr ? "Quel résultat produit ce code ?" : es ? "¿Qué resultado produce este código?" : q.prompt}</h1><pre className="mb-6 overflow-auto rounded-2xl bg-slate-950 p-6 text-lg text-slate-100"><code>{q.code}</code></pre>
    <div className="grid gap-3 sm:grid-cols-2">{q.options.map(o => <button key={o} disabled={checked} onClick={() => setSelected(o)} className={optionClass(selected === o)}>{o}</button>)}</div>{checked && <p role="status" className={`mt-5 rounded-xl p-4 font-bold ${selected === q.answer ? "bg-emerald-50 text-emerald-800" : "bg-amber-50 text-amber-900"}`}>{selected === q.answer ? (fr ? "Correct ! Louis est impressionné." : es ? "¡Correcto! Louis está impresionado." : "Correct! Louis is impressed.") : (fr ? `La réponse est : ${q.answer}` : es ? `La respuesta es: ${q.answer}` : `The answer is: ${q.answer}`)}</p>}
    <button disabled={!selected} onClick={() => checked ? next() : setChecked(true)} className={`${primaryButton} mt-7`}>{checked ? (index === 4 ? (fr ? "VOIR MON RÉSULTAT" : es ? "VER MI RESULTADO" : "SEE MY RESULT") : (fr ? "SUIVANT" : es ? "SIGUIENTE" : "NEXT")) : (fr ? "VÉRIFIER" : es ? "COMPROBAR" : "CHECK")}</button>
  </OnboardingShell>;
}
