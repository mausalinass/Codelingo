    ...
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 xl:gap-8 items-start w-full">
        ...
        <div className="lg:col-span-7 xl:col-span-7 flex flex-col gap-6 w-full order-2 lg:order-1">
          {(dashboardError || personalityError) && <div role="alert" className="rounded-xl border border-red-300 bg-red-50 text-red-900 p-4">Unable to load saved progress or personality. Check the API connection and try again. <button className="font-bold underline" onClick={() => { void refetch(); void refetchPersonality(); }}>Retry</button></div>}
