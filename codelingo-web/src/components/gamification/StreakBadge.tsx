...
export function StreakScale({ current }: { current: number }) {
  ...
  return <section className="rounded-2xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800/80" aria-label="Learning streak colors">
    <div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-wider">Your learning streak</p><p className="text-[10px] font-semibold text-slate-500">Every day counts</p></div><strong className="text-sm">{current} days</strong></div>
