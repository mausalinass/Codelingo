...
export const ProgressRecord: React.FC<ProgressRecordProps> = ({
  ...
  const completedLessonsTotal =
    dashboard?.courses.reduce((acc, c) => acc + c.completedLessons, 0) ?? 0;
  const totalPossibleLessons = dashboard?.courses.reduce((sum, c) => sum + c.totalLessons, 0) ?? 0;
  ...
  const milestones = [
    ...
    },
    {
      ...
      unlocked: (dashboard?.courses.filter(c => c.completedLessons > 0).length ?? 0) >= 2,
      icon: Trophy,
      color:
